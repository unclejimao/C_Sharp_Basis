...
- 方法调用时栈内存的分配
  - 对stack frame的分析
    ...
    stack frame指的是方法在被调用时在栈内存中的布局

    **这块比较难理解，但是很重要，要对照第9节视频反复揣摩**

## 操作符详解
### 操作符概览
操作符（Operator）也译为“运算符”</br>
操作符是用来操作数据的，被操作的数据称作操作数（Operand）</br>
下表所示的操作符从上到下优先级依次降低

![](https://raw.githubusercontent.com/unclejimao/pictureBed/master/Operator.png)
...
- 操作符不能脱离与它相关联的数据类型
 ...
 ### 操作符的优先级与运算顺序
- 除了上表列出的优先级顺序外，可以使用圆括号()来提高被括起来的表达式的优先级，圆括号可以嵌套
- 同优先级操作符的运算顺序
  - 除了带有赋值功能的操作符，同优先级操作符都是从左向右进行运算
  - 带有赋值功能的操作符的运算顺序是从右向左
  - 与数学运算不同，计算机语言的同优先级运算没有“结合律”
  
    比如，```3+4+5``` 只能理解为```Add(Add(3,4),5)``` ,不能理解为```Add(3,Add(4,5))```

## var关键字

var来源于单词variable，其作用就是帮助我们声明隐式类型的变量。

C#是一种强类型语言，显示类型变量的声明如```int x=10;```直接告诉编译器所需要的变量数据类型；

而隐式类型变量声明用```var x；```意思是告诉编译器我需要一个变量x，但是它的类型暂时不知道，请编译器根据它的赋值去推断变量x的类型

var 声明的变量一旦被编译器认为是什么类型，就一直是这个类型，不能再给这个变量赋其他类型的值。